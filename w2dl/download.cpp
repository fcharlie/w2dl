...
public:
  ...
  bool Create(std::wstring_view name) {
    ...
    part = file + L".part";
    hFile =
        CreateFileW(part.c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ,
  ...
  bool Finish() {
    ...
    return MoveFileExW(part.c_str(), npath.c_str(), MOVEFILE_COPY_ALLOWED) ==
           TRUE;
  }
...
struct URI_INFO {
  ...
  bool CrackURI(std::wstring_view url) {
    ...
    if (WinHttpCrackUrl(url.data(), static_cast<DWORD>(url.size()), ICU_ESCAPE,
                        &uc) != TRUE) {
      return false;
    }
    nScheme = uc.nScheme;
...
struct NetObject {
  ...
  operator HINTERNET() { return hNet; }
  operator bool() { return hNet != nullptr; }
  NetObject() {
    if (hNet) {
...
bool HttpGet(const std::wstring &url, const std::wstring &file) {
  ...
  NetObject hNet =
      WinHttpOpen(W2DL_USERAGENT, WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
  ...
  if (!WinHttpSetOption(hNet, WINHTTP_OPTION_REDIRECT_POLICY, &dwOption,
                        sizeof(DWORD))) {
    return false;
  }
  // HTTP2 support
  dwOption = WINHTTP_PROTOCOL_FLAG_HTTP2;
  if (!WinHttpSetOption(hNet, WINHTTP_OPTION_ENABLE_HTTP_PROTOCOL, &dwOption,
                        sizeof(dwOption))) {
    fprintf(stderr, "HTTP2 is not supported\n");
  }
  NetObject hConnect =
      WinHttpConnect(hNet, ui.szHostName, (INTERNET_PORT)ui.Port, 0);
  if (!hConnect) {
    return false;
  }
  ...
  NetObject hRequest = WinHttpOpenRequest(hConnect, L"GET", ui.PathQuery.data(),
  ...
  do {
    ...
    while (dwSizeN > 0) {
      DWORD dwRead = 0;
      if (!WinHttpReadData(hRequest, (LPVOID)recvbuf,
                           MinWarp(sizeof(recvbuf), dwSizeN), &dwRead)) {
        break;
      }
