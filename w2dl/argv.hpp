...
ErrorResult Integer_from_wchars(std::wstring_view wsv, Integer &ov, const int base) {
	...
	ov = static_cast<Unsigned>(_value);	// implementation-defined for negative, N4713 7.8 [conv.integral]/3
	return ErrorResult{};
}
...
public:
	...
	using ArgumentCallback = std::function<bool(int, const wchar_t *optarg, const wchar_t *raw)>;
	ErrorResult ParseArgument(const std::vector<option> &opts, const ArgumentCallback &callback) {
	...
	const std::vector<std::wstring_view>&UnresolvedArgs()const {
		return uargs;
	}
private:
	int argc_;
